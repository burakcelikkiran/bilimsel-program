...
<template>
  ...
  >
        ...
        <div class="bg-gradient-to-br from-gray-700 to-gray-800 rounded-xl p-6 text-white shadow-lg border border-gray-600">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-bold">{{ enhancedStats.total_capacity || 0 }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-gray-600 to-gray-700 rounded-xl p-6 text-white shadow-lg border border-gray-500">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-bold">{{ enhancedStats.active_sessions || 0 }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-gray-500 to-gray-600 rounded-xl p-6 text-white shadow-lg border border-gray-400">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-bold">{{ enhancedStats.event_days || 0 }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white dark:bg-gray-900 shadow-xl rounded-2xl overflow-hidden border border-gray-200/50 dark:border-gray-800/50">
      ...
      <div class="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 px-8 py-6 border-b border-gray-200 dark:border-gray-800">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
          ...
          <div class="flex items-center space-x-3">
            ...
            <!-- Create Button -->
            <Link
              :href="safeRoute('admin.venues.create', '/admin/venues/create')"
      ...
      <!-- List View -->
      <div v-else-if="viewMode === 'list' && displayedVenues.length > 0" class="overflow-hidden">
        <!-- Bulk Actions -->
        <div 
          ...
          <table class="w-full divide-y divide-gray-200 dark:divide-gray-800">
            <thead class="bg-gray-50 dark:bg-gray-800/50">
              <tr>
                ...
                <!-- Capacity Header -->
                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-all duration-200 w-32" @click="handleSort('capacity')">
                  <div class="flex items-center space-x-1">
                    <span>Kapasite</span>
                    <div class="flex flex-col">
                      <ChevronUpIcon class="h-3 w-3 transition-colors duration-200" :class="sortField === 'capacity' && sortDirection === 'asc' ? 'text-gray-600' : 'text-gray-300'" />
                      <ChevronDownIcon class="h-3 w-3 -mt-1 transition-colors duration-200" :class="sortField === 'capacity' && sortDirection === 'desc' ? 'text-gray-600' : 'text-gray-300'" />
                    </div>
                  </div>
                </th>
              ...
              >
                ...
                <td class="px-6 py-6">
                  <div class="flex items-center">
                    ...
                    <div class="ml-4">
                      <Link
                        :href="safeRoute('admin.venues.show', `/admin/venues/${venue.id}`, venue.id)"
                      ...
                      <p v-if="venue.display_name && venue.display_name !== venue.name" class="text-gray-600 dark:text-gray-400 mt-1 text-sm">
                ...
                <td class="px-6 py-6 w-48">
                  <div class="text-sm">
                    <div v-if="venue.event_day" class="font-medium text-gray-900 dark:text-white">
                      {{ venue.event_day.display_name }}
                    </div>
                    <div v-else class="text-gray-500 dark:text-gray-400">
                      Etkinlik günü belirtilmemiş
                    </div>
                    <div v-if="venue.event_day?.event" class="text-gray-500 dark:text-gray-400 flex items-center mt-1">
                ...
                <td class="px-6 py-6 w-32">
                  <div class="text-sm">
                    <div v-if="venue.capacity" class="font-medium text-gray-900 dark:text-white">
                ...
                <td class="px-6 py-6 w-32">
                  <div class="text-sm">
                    <div class="font-medium text-gray-900 dark:text-white">
                      {{ venue.program_sessions_count || 0 }}
                    </div>
                ...
                <td class="px-6 py-6 w-60">
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    <!-- Edit -->
                    <Link
                      :href="safeRoute('admin.venues.edit', `/admin/venues/${venue.id}/edit`, venue.id)"
                    ...
                    <div class="relative">
                      <button
                        @click="toggleActionsMenu(venue.id)"
                      ...
                      >
                        <div class="py-1">
                          ...
                          <!-- Sessions Link -->
                          <Link
                            v-if="venue.program_sessions_count > 0"
                            :href="safeRoute('admin.program-sessions.index', `/admin/program-sessions?venue_id=${venue.id}`)"
                            class="flex items-center w-full px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
                          >
      ...
      <!-- Grid View -->
      <div v-else-if="viewMode === 'grid' && displayedVenues.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
        <div
          v-for="venue in displayedVenues"
          :key="venue.id"
        ...
        >
          ...
          <div class="flex items-start justify-between mb-4">
            <div class="flex items-center space-x-3">
              ...
              <div class="flex-1">
                ...
                <p v-if="venue.event_day" class="text-sm text-gray-500 dark:text-gray-400">
                  {{ venue.event_day.display_name }}
                </p>
              </div>
            </div>
            
            <span v-if="venue.capacity" class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300">
              {{ formatCapacity(venue.capacity) }}
            </span>
          </div>
          ...
          <div class="space-y-3 text-sm text-gray-600 dark:text-gray-400">
            <div v-if="venue.event_day?.event" class="flex items-center">
      ...
      <div v-else class="py-16">
        <div class="text-center">
          ...
          <h3 class="mt-6 text-xl font-semibold text-gray-900 dark:text-white">
            {{ searchQuery || hasActiveFilters ? 'Sonuç bulunamadı' : 'Henüz salon yok' }}
          </h3>
      ...
      <!-- Enhanced Pagination -->
      <div v-if="venues?.last_page > 1" class="bg-gray-50 dark:bg-gray-800/50 px-8 py-6 border-t border-gray-200 dark:border-gray-800">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-4">
            <p class="text-sm text-gray-700 dark:text-gray-300">
              <span class="font-medium">{{ venues.from || 0 }}</span>
              -
              <span class="font-medium">{{ venues.to || 0 }}</span>
              arası, toplam
              <span class="font-medium">{{ venues.total || 0 }}</span>
              sonuç
            </p>
...
const enhancedStats = computed(() => {
  ...
  return {
    ...
    total_capacity: venues.reduce((sum, venue) => sum + (venue.capacity || 0), 0),
    active_sessions: venues.reduce((sum, venue) => sum + (venue.program_sessions_count || 0), 0),
    event_days: new Set(venues.map(v => v.event_day_id).filter(Boolean)).size
  }
})
...
const isAllSelected = computed(() => {
  return displayedVenues.value.length > 0 && selectedVenues.value.length === displayedVenues.value.length
})

const isIndeterminate = computed(() => {
  return selectedVenues.value.length > 0 && selectedVenues.value.length < displayedVenues.value.length
})
...
const updateUrl = (params) => {
  ...
  router.get(safeRoute('admin.venues.index', '/admin/venues'), {
    ...existingParams,
    ...params,
...
const duplicateVenue = (venue) => {
  ...
  router.post(safeRoute('admin.venues.duplicate', `/admin/venues/${venue.id}/duplicate`, venue.id), {}, {
    onError: () => {
...
const deleteVenue = (venue) => {
  ...
  confirmDialog.value = {
    show: true,
    title: 'Salonu Sil',
    message: `"${venue.name}" salonunu silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`,
    type: 'danger',
    callback: () => {
      router.delete(safeRoute('admin.venues.destroy', `/admin/venues/${venue.id}`, venue.id), {
        onSuccess: () => {
...
// Bulk Actions
const bulkDuplicate = () => {
  router.post(safeRoute('admin.venues.bulk-duplicate', '/admin/venues/bulk-duplicate'), { 
...
const bulkDelete = () => {
  confirmDialog.value = {
    ...
    type: 'danger',
    callback: () => {
      router.delete(safeRoute('admin.venues.bulk-destroy', '/admin/venues/bulk-destroy'), {
