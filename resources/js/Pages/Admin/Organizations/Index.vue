...
<template>
  ...
  >
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl p-6 text-white shadow-lg border border-gray-700">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <p class="text-gray-300 text-sm">Toplam Organizasyon</p>
        ...
        <div class="bg-gradient-to-br from-gray-600 to-gray-700 rounded-xl p-6 text-white shadow-lg border border-gray-500">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <p class="text-gray-300 text-sm">Etkinlikli</p>
              <p class="text-2xl font-bold">{{ enhancedStats.withEvents }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-gray-500 to-gray-600 rounded-xl p-6 text-white shadow-lg border border-gray-400">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-bold">{{ enhancedStats.totalParticipants }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white dark:bg-gray-900 shadow-xl rounded-2xl overflow-hidden border border-gray-200/50 dark:border-gray-800/50">
      ...
      <div class="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 px-8 py-6 border-b border-gray-200 dark:border-gray-800">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
          ...
          <div class="flex items-center space-x-3">
            ...
            <!-- Create Button -->
            <Link
              :href="safeRoute('admin.organizations.create', '/admin/organizations/create')"
        ...
        <div class="mt-6 flex flex-col lg:flex-row lg:items-center space-y-4 lg:space-y-0 lg:space-x-4">
          ...
          <div class="flex items-center space-x-3">
            ...
            >
              ...
              <option value="events_count">Etkinlik Sayısı</option>
              <option value="participants_count">Katılımcı Sayısı</option>
            </select>
            ...
            <button
              ...
              @click="clearFilters"
              class="px-4 py-3 text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 border border-gray-300 dark:border-gray-700 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-600 transition-all duration-200"
      ...
      <!-- List View -->
      <div v-else-if="viewMode === 'list' && displayedOrganizations.length > 0" class="overflow-hidden">
        <!-- Bulk Actions -->
        <div 
        ...
        >
          <div class="flex items-center justify-between">
            <p class="text-gray-800 dark:text-gray-200 font-medium">
              {{ selectedOrganizations.length }} organizasyon seçildi
            </p>
            <div class="flex items-center space-x-3">
              <button
          ...
          <table class="w-full divide-y divide-gray-200 dark:divide-gray-800">
            <thead class="bg-gray-50 dark:bg-gray-800/50">
              <tr>
                ...
                <!-- Events Header -->
                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-all duration-200 w-32" @click="handleSort('events_count')">
                  <div class="flex items-center space-x-1">
                    <span>Etkinlikler</span>
                    <div class="flex flex-col">
                      <ChevronUpIcon class="h-3 w-3 transition-colors duration-200" :class="sortField === 'events_count' && sortDirection === 'asc' ? 'text-gray-600' : 'text-gray-300'" />
                      <ChevronDownIcon class="h-3 w-3 -mt-1 transition-colors duration-200" :class="sortField === 'events_count' && sortDirection === 'desc' ? 'text-gray-600' : 'text-gray-300'" />
                    </div>
                  </div>
                </th>
                
                <!-- Participants Header -->
                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-all duration-200 w-32" @click="handleSort('participants_count')">
                  <div class="flex items-center space-x-1">
                    <span>Katılımcılar</span>
                    <div class="flex flex-col">
                      <ChevronUpIcon class="h-3 w-3 transition-colors duration-200" :class="sortField === 'participants_count' && sortDirection === 'asc' ? 'text-gray-600' : 'text-gray-300'" />
                      <ChevronDownIcon class="h-3 w-3 -mt-1 transition-colors duration-200" :class="sortField === 'participants_count' && sortDirection === 'desc' ? 'text-gray-600' : 'text-gray-300'" />
                    </div>
                  </div>
                </th>
              ...
              >
                ...
                <td class="px-6 py-6">
                  <div class="flex items-center">
                    ...
                    <div class="ml-4">
                      <Link
                        :href="safeRoute('admin.organizations.show', `/admin/organizations/${organization.id}`, organization.id)"
                      ...
                      <p v-if="organization.description" class="text-gray-600 dark:text-gray-400 mt-1 text-sm line-clamp-1 max-w-md">
                ...
                <td class="px-6 py-6 w-48">
                  <div class="text-sm">
                    <div v-if="organization.contact_email" class="flex items-center mb-1">
                      <EnvelopeIcon class="h-4 w-4 mr-2 text-gray-400" />
                      <a :href="`mailto:${organization.contact_email}`" class="text-gray-600 dark:text-gray-400 hover:underline">
                        {{ organization.contact_email }}
                      </a>
                    </div>
                    <div v-if="organization.contact_phone" class="flex items-center">
                      <PhoneIcon class="h-4 w-4 mr-2 text-gray-400" />
                      <span class="text-gray-900 dark:text-white">{{ organization.contact_phone }}</span>
                    </div>
                  </div>
                </td>
                ...
                <td class="px-6 py-6 w-32">
                  ...
                  >
                    <svg
                      :class="[
                        'mr-1.5 h-2 w-2',
                        organization.is_active ? 'text-green-400' : 'text-red-400'
                      ]"
                      fill="currentColor"
                      viewBox="0 0 8 8"
                    >
                ...
                <td class="px-6 py-6 w-32">
                  <div class="text-center">
                    ...
                    <div v-if="organization.active_events" class="text-xs text-gray-500 dark:text-gray-400">
                      {{ organization.active_events }} aktif
                    </div>
                  </div>
                </td>
                ...
                <td class="px-6 py-6 w-60">
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    <Link
                      ...
                      :href="safeRoute('admin.organizations.edit', `/admin/organizations/${organization.id}/edit`, organization.id)"
                    ...
                    <div class="relative">
                      <button
                        @click="toggleActionsMenu(organization.id)"
                      ...
                      <!-- Dropdown Menu -->
                      <div
                        v-if="showActionsMenu === organization.id"
                      ...
                      >
                        <div class="py-1">
                          ...
                          >
                            <CheckCircleIcon v-if="!organization.is_active" class="h-4 w-4 mr-2" />
                            <XCircleIcon v-else class="h-4 w-4 mr-2" />
                            {{ organization.is_active ? 'Pasifleştir' : 'Aktifleştir' }}
                          </button>

                          <!-- Delete -->
                          <button
                            v-if="organization.can_delete"
      ...
      <!-- Grid View -->
      <div v-else-if="viewMode === 'grid' && displayedOrganizations.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
        <div
          v-for="organization in displayedOrganizations"
          :key="organization.id"
        ...
        >
          ...
          <div class="flex items-start justify-between mb-4">
            <div class="flex items-center space-x-3">
              ...
              <div class="flex-1">
                ...
                <p v-if="organization.contact_email" class="text-sm text-gray-500 dark:text-gray-400">
                  {{ organization.contact_email }}
                </p>
              </div>
            </div>
          ...
          <div class="space-y-3 text-sm text-gray-600 dark:text-gray-400">
            <div class="flex items-center justify-between">
              ...
              <span class="font-medium text-gray-900 dark:text-white">{{ organization.events_count || 0 }}</span>
            </div>
            ...
            <div class="flex items-center justify-between">
              ...
              <span class="font-medium text-gray-900 dark:text-white">{{ organization.participants_count || 0 }}</span>
            </div>
      ...
      <div v-else class="py-16">
        <div class="text-center">
          ...
          <h3 class="mt-6 text-xl font-semibold text-gray-900 dark:text-white">
            {{ searchQuery || hasActiveFilters ? 'Sonuç bulunamadı' : 'Henüz organizasyon yok' }}
          </h3>
      ...
      <!-- Enhanced Pagination -->
      <div v-if="organizations?.last_page > 1" class="bg-gray-50 dark:bg-gray-800/50 px-8 py-6 border-t border-gray-200 dark:border-gray-800">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-4">
            <p class="text-sm text-gray-700 dark:text-gray-300">
              <span class="font-medium">{{ organizations.from || 0 }}</span>
              -
              <span class="font-medium">{{ organizations.to || 0 }}</span>
              arası, toplam
              <span class="font-medium">{{ organizations.total || 0 }}</span>
              sonuç
            </p>
...
const enhancedStats = computed(() => {
  ...
  const totalParticipants = data.reduce((sum, org) => sum + (org.participants_count || 0), 0)
  
  return {
    total: props.stats?.total || props.organizations?.total || data.length || 0,
    active: props.stats?.active || data.filter(org => org.is_active).length || 0,
    withEvents: props.stats?.withEvents || data.filter(org => (org.events_count || 0) > 0).length || 0,
...
const isAllSelected = computed(() => {
  return displayedOrganizations.value.length > 0 && selectedOrganizations.value.length === displayedOrganizations.value.length
})

const isIndeterminate = computed(() => {
  return selectedOrganizations.value.length > 0 && selectedOrganizations.value.length < displayedOrganizations.value.length
})
...
const updateUrl = (params) => {
  ...
  router.get(safeRoute('admin.organizations.index', '/admin/organizations'), {
    ...existingParams,
    ...params,
...
const toggleSelectAll = () => {
  if (isAllSelected.value) {
    ...
    selectedOrganizations.value = displayedOrganizations.value.map(organization => organization.id)
  }
}

const toggleActionsMenu = (organizationId) => {
  showActionsMenu.value = showActionsMenu.value === organizationId ? null : organizationId
}
...
const deleteOrganization = (organization) => {
  ...
  confirmDialog.value = {
    ...
    type: 'danger',
    callback: () => {
      router.delete(safeRoute('admin.organizations.destroy', `/admin/organizations/${organization.id}`, organization.id), {
        onSuccess: () => {
...
const toggleStatus = (organization) => {
  showActionsMenu.value = null
  router.patch(safeRoute('admin.organizations.toggle-status', `/admin/organizations/${organization.id}/toggle-status`, organization.id), {}, {
    preserveScroll: true,
    onError: () => {
...
const bulkToggleStatus = () => {
  router.patch(safeRoute('admin.organizations.bulk-toggle-status', '/admin/organizations/bulk-toggle-status'), { 
...
const bulkDelete = () => {
  confirmDialog.value = {
    ...
    type: 'danger',
    callback: () => {
      router.delete(safeRoute('admin.organizations.bulk-destroy', '/admin/organizations/bulk-destroy'), {
