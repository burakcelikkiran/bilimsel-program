    ...
    <div class="w-full space-y-8">
                ...
                <div class="flex items-end justify-between">
                  <div class="flex items-center space-x-6">
                    ...
                    <div>
                      ...
                      <p v-if="safeParticipant.title" class="text-slate-200 text-lg">{{ safeParticipant.title }}</p>
                      <p v-if="safeParticipant.affiliation" class="text-slate-300 text-sm">{{ safeParticipant.affiliation }}</p>
                    </div>
                  </div>
          ...
          <div class="px-8 py-6 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
            <div class="flex flex-wrap items-center gap-6">
              ...
              <div class="flex items-center space-x-2">
                <span class="text-sm font-medium text-slate-500 dark:text-slate-400">Roller:</span>
                <div class="flex space-x-2">
                  <span
                    v-if="safeParticipant.is_speaker"
                    class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-slate-200 text-slate-800 dark:bg-slate-700 dark:text-slate-300"
                  >
                  ...
                  <span
                    ...
                    class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-slate-300 text-slate-900 dark:bg-slate-600 dark:text-slate-200"
                  >
                  ...
                  <span
                    ...
                    class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-300"
                  >
              ...
              <div v-if="safeParticipant.email" class="flex items-center space-x-2">
                ...
                <div>
                  <a :href="`mailto:${safeParticipant.email}`" class="text-sm font-medium text-slate-600 dark:text-slate-400 hover:underline">
              ...
              <div v-if="safeParticipant.phone" class="flex items-center space-x-2">
                ...
                <div>
                  <a :href="`tel:${safeParticipant.phone}`" class="text-sm font-medium text-slate-600 dark:text-slate-400 hover:underline">
              ...
              <div class="flex items-center space-x-2">
                ...
                <div>
                  <div class="text-sm font-medium text-slate-900 dark:text-white">{{ safeParticipant.organization?.name || 'Organizasyon yok' }}</div>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-4 gap-8">
        ...
        <div class="xl:col-span-3 space-y-8">
          <!-- Biography -->
          <div v-if="safeParticipant.bio" class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            <h3 class="text-lg font-semibold text-slate-900 dark:text-white mb-4">Biyografi</h3>
            <div class="prose dark:prose-invert max-w-none">
              <p class="text-slate-700 dark:text-slate-300 leading-relaxed">{{ safeParticipant.bio }}</p>
            </div>
          </div>
          ...
          <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
              <h3 class="text-lg font-semibold text-slate-900 dark:text-white">Sunumlar</h3>
            ...
            <!-- Presentations List -->
            <div v-if="safeParticipant.presentations?.length > 0" class="divide-y divide-slate-200 dark:divide-slate-700">
              <div
            ...
            <div v-else class="text-center py-16">
              ...
              <h4 class="text-lg font-medium text-slate-900 dark:text-slate-100 mb-2">Henüz sunum yok</h4>
              <p class="text-slate-500 dark:text-slate-400">Bu katılımcının henüz kayıtlı sunumu bulunmuyor.</p>
            </div>
          </div>
          ...
          <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
              <h3 class="text-lg font-semibold text-slate-900 dark:text-white">Modere Ettiği Oturumlar</h3>
              <span class="text-sm text-slate-500 dark:text-slate-400">
            ...
            <div v-if="safeParticipant.moderated_sessions?.length > 0" class="divide-y divide-slate-200 dark:divide-slate-700">
              <div
            ...
            <div v-else class="text-center py-16">
              ...
              <h4 class="text-lg font-medium text-slate-900 dark:text-slate-100 mb-2">Henüz moderasyon yok</h4>
              <p class="text-slate-500 dark:text-slate-400">Bu katılımcının henüz modere ettiği oturum bulunmuyor.</p>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            <h3 class="text-lg font-semibold text-slate-900 dark:text-white mb-4">Katılımcı Detayları</h3>
            <div class="space-y-4">
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-2">
                  <UserIcon class="h-5 w-5 text-slate-500" />
                  <span class="text-sm text-slate-600 dark:text-slate-400">Ad Soyad</span>
                </div>
                <span class="text-sm font-semibold text-slate-900 dark:text-white">{{ safeParticipant.full_name }}</span>
              </div>
              
              <div v-if="safeParticipant.title" class="flex items-center justify-between">
                <div class="flex items-center space-x-2">
                  <AcademicCapIcon class="h-5 w-5 text-slate-500" />
                  <span class="text-sm text-slate-600 dark:text-slate-400">Ünvan</span>
                </div>
                <span class="text-sm font-semibold text-slate-900 dark:text-white">{{ safeParticipant.title }}</span>
              </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="text-sm font-semibold text-slate-900 dark:text-white">{{ safeParticipant.presentations?.length || 0 }}</span>
              </div>
              
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-2">
                  <UserGroupIcon class="h-5 w-5 text-slate-500" />
                  <span class="text-sm text-slate-600 dark:text-slate-400">Moderasyon Sayısı</span>
                </div>
                <span class="text-sm font-semibold text-slate-900 dark:text-white">{{ safeParticipant.moderated_sessions?.length || 0 }}</span>
              </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="text-sm font-semibold text-slate-900 dark:text-white">{{ formatDate(safeParticipant.created_at) }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            ...
            <div class="space-y-3">
              <div v-if="safeParticipant.email" class="flex items-center space-x-3 p-3 border border-slate-200 dark:border-slate-700 rounded-lg">
                ...
                <div class="flex-1">
                  ...
                  <a :href="`mailto:${safeParticipant.email}`" class="text-sm text-slate-600 dark:text-slate-400 hover:underline">
                    {{ safeParticipant.email }}
                  </a>
                </div>
              </div>
              
              <div v-if="safeParticipant.phone" class="flex items-center space-x-3 p-3 border border-slate-200 dark:border-slate-700 rounded-lg">
                <div class="w-8 h-8 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center">
                ...
                <div class="flex-1">
                  ...
                  <a :href="`tel:${safeParticipant.phone}`" class="text-sm text-slate-600 dark:text-slate-400 hover:underline">
                    {{ safeParticipant.phone }}
                  </a>
                </div>
              </div>
          ...
          <div v-if="safeParticipant.organization || safeParticipant.affiliation" class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            <h3 class="text-lg font-semibold text-slate-900 dark:text-white mb-4">Organizasyon Bilgileri</h3>
            <div class="space-y-3">
              <div v-if="safeParticipant.organization" class="flex items-center space-x-3 p-3 border border-slate-200 dark:border-slate-700 rounded-lg">
                <div class="w-8 h-8 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center">
                  <BuildingOfficeIcon class="w-4 h-4 text-slate-600 dark:text-slate-400" />
                </div>
                <div class="flex-1">
                  <div class="text-sm font-medium text-slate-900 dark:text-white">Organizasyon</div>
                  <div class="text-sm text-slate-600 dark:text-slate-400">{{ safeParticipant.organization.name }}</div>
                </div>
              </div>
              
              <div v-if="safeParticipant.affiliation" class="flex items-center space-x-3 p-3 border border-slate-200 dark:border-slate-700 rounded-lg">
                <div class="w-8 h-8 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center">
                  <AcademicCapIcon class="w-4 h-4 text-slate-600 dark:text-slate-400" />
                </div>
                <div class="flex-1">
                  <div class="text-sm font-medium text-slate-900 dark:text-white">Kurum/Affiliation</div>
                  <div class="text-sm text-slate-600 dark:text-slate-400">{{ safeParticipant.affiliation }}</div>
                </div>
              </div>
            </div>
          </div>
