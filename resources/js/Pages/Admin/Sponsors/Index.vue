...
<template>
  ...
  >
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg p-4 text-white shadow border border-slate-700">
          <div class="flex items-center">
            ...
            <div class="ml-3">
              <p class="text-slate-300 text-xs">Toplam Sponsor</p>
              <p class="text-lg font-bold">{{ enhancedStats.total }}</p>
            </div>
          </div>
        </div>
        
        <div class="bg-gradient-to-br from-slate-700 to-slate-800 rounded-lg p-4 text-white shadow border border-slate-600">
          <div class="flex items-center">
            <div class="p-2 bg-white/10 rounded-lg backdrop-blur-sm">
              <CheckCircleIcon class="h-5 w-5" />
            </div>
            <div class="ml-3">
              <p class="text-slate-300 text-xs">Aktif</p>
              <p class="text-lg font-bold">{{ enhancedStats.active }}</p>
            </div>
          </div>
        </div>
        
        <div class="bg-gradient-to-br from-slate-600 to-slate-700 rounded-lg p-4 text-white shadow border border-slate-500">
          <div class="flex items-center">
            <div class="p-2 bg-white/10 rounded-lg backdrop-blur-sm">
              <PhotoIcon class="h-5 w-5" />
            </div>
            <div class="ml-3">
              <p class="text-slate-300 text-xs">Logolu</p>
              <p class="text-lg font-bold">{{ enhancedStats.with_logo }}</p>
            </div>
          </div>
        </div>
        
        <div class="bg-gradient-to-br from-slate-500 to-slate-600 rounded-lg p-4 text-white shadow border border-slate-400">
          <div class="flex items-center">
            <div class="p-2 bg-white/10 rounded-lg backdrop-blur-sm">
              <StarIcon class="h-5 w-5" />
            </div>
            <div class="ml-3">
              <p class="text-slate-300 text-xs">Ana Sponsor</p>
              <p class="text-lg font-bold">{{ enhancedStats.main_sponsors }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- Modern Data Table -->
    <div class="bg-white dark:bg-slate-800 shadow-lg rounded-lg overflow-visible border border-slate-200/50 dark:border-slate-800/50">
      <!-- Enhanced Header -->
      <div class="bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 px-4 py-4 border-b border-slate-200 dark:border-slate-800">
        ...
        <div class="mt-4 flex flex-col lg:flex-row lg:items-center space-y-3 lg:space-y-0 lg:space-x-3">
          <!-- Search Bar -->
      ...
      <div v-if="loading" class="flex items-center justify-center py-16">
        <div class="flex flex-col items-center space-y-4">
          ...
          <p class="text-slate-600 dark:text-slate-400 font-medium">Yükleniyor...</p>
        </div>
      </div>

      <!-- List View -->
      <div v-else-if="viewMode === 'list' && displayedSponsors.length > 0" class="overflow-visible">
        <!-- Bulk Actions -->
        <div 
          ...
          <table class="w-full divide-y divide-slate-200 dark:divide-slate-800">
            <thead class="bg-slate-50 dark:bg-slate-800/50">
              <tr>
                ...
                <!-- Sponsor Header -->
                <th class="px-3 py-2 text-left text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-all duration-200" @click="handleSort('name')">
                  <div class="flex items-center space-x-1">
                    <span>Sponsor</span>
                    <div class="flex flex-col">
                      <ChevronUpIcon class="h-3 w-3 transition-colors duration-200" :class="sortField === 'name' && sortDirection === 'asc' ? 'text-slate-600' : 'text-slate-300'" />
                      <ChevronDownIcon class="h-3 w-3 -mt-1 transition-colors duration-200" :class="sortField === 'name' && sortDirection === 'desc' ? 'text-slate-600' : 'text-slate-300'" />
                    </div>
                  </div>
                </th>
                
                <!-- Type Header -->
                <th class="px-3 py-2 text-left text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-28">
                  Tür
                </th>
                
                <!-- Event Header -->
                <th class="px-3 py-2 text-left text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-40">
                  Etkinlik
                </th>
                
                <!-- Status Header -->
                <th class="px-3 py-2 text-left text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-24">
                  Durum
                </th>
                
                <!-- Sort Order Header -->
                <th class="px-3 py-2 text-left text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-all duration-200 w-16" @click="handleSort('sort_order')">
                  <div class="flex items-center space-x-1">
                    <span>Sıra</span>
                    <div class="flex flex-col">
                      <ChevronUpIcon class="h-3 w-3 transition-colors duration-200" :class="sortField === 'sort_order' && sortDirection === 'asc' ? 'text-slate-600' : 'text-slate-300'" />
                      <ChevronDownIcon class="h-3 w-3 -mt-1 transition-colors duration-200" :class="sortField === 'sort_order' && sortDirection === 'desc' ? 'text-slate-600' : 'text-slate-300'" />
                    </div>
                  </div>
                </th>
                
                <!-- Actions Header -->
                <th class="px-3 py-2 text-center text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-32">
                  İşlemler
                </th>
              </tr>
            </thead>
            
           <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-800">
              <tr 
              ...
              >
                  ...
                  <div class="flex items-center">
                    <div class="flex-shrink-0 h-8 w-8">
                      <img
                        ...
                        :alt="sponsor.name"
                        class="h-8 w-8 rounded-lg object-cover border border-slate-200 dark:border-slate-600"
                      />
                      <div
                        v-else
                        class="h-8 w-8 rounded-lg bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-800 flex items-center justify-center"
                      >
                    ...
                    <div class="ml-2.5 min-w-0">
                      ...
                      <p v-if="sponsor.website" class="text-slate-500 dark:text-slate-400 text-xs truncate max-w-xs hover:underline">
                        <a :href="sponsor.website" target="_blank">
                          {{ sponsor.website }}
                        </a>
                      </p>
                      <div v-if="sponsor.description" class="text-slate-500 dark:text-slate-400 text-xs line-clamp-1 max-w-xs">
                        {{ sponsor.description }}
                      </div>
                    </div>
                  </div>
                ...
                <td class="px-3 py-2 w-40">
                  <div class="text-xs">
                    <div v-if="sponsor.event" class="font-medium text-slate-900 dark:text-white line-clamp-1">
                      {{ sponsor.event.name }}
                    </div>
                    <div v-else class="text-slate-500 dark:text-slate-400">
                      Etkinlik belirtilmemiş
                    </div>
                    <div v-if="sponsor.event?.start_date" class="text-slate-500 dark:text-slate-400 flex items-center mt-0.5">
                ...
                <td class="px-3 py-2 w-16">
                  <div class="text-xs font-medium text-slate-900 dark:text-white text-center">
                    {{ sponsor.sort_order || '-' }}
                  </div>
                </td>
        ...
        >
          ...
          <div class="flex items-start justify-between mb-4">
            <div class="flex items-center space-x-2">
              <div class="flex-shrink-0 h-12 w-12">
                ...
                >
                  <BuildingOfficeIcon class="w-6 h-6 text-slate-400" />
                </div>
              </div>
              <div class="flex-1">
                <h4 class="text-lg font-semibold text-slate-900 dark:text-white line-clamp-1">
                  {{ sponsor.name }}
                </h4>
                <p v-if="sponsor.event" class="text-sm text-slate-500 dark:text-slate-400">
                  {{ sponsor.event.name }}
                </p>
              </div>
            </div>
          ...
          <div class="space-y-3 text-sm text-slate-600 dark:text-slate-400">
            ...
            <div v-if="sponsor.website" class="flex items-center">
              ...
              <a :href="sponsor.website" target="_blank" class="text-slate-600 hover:underline truncate">
                {{ sponsor.website }}
              </a>
            </div>
      ...
      <div v-else class="py-16">
        <div class="text-center">
          <div class="mx-auto h-24 w-24 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900 rounded-2xl flex items-center justify-center">
      ...
      <!-- Enhanced Pagination -->
      <div v-if="sponsors?.last_page > 1" class="bg-slate-50 dark:bg-slate-800/50 px-4 py-3 border-t border-slate-200 dark:border-slate-800">
        <div class="flex items-center justify-between">
...
const getSponsorTypeClasses = (sponsorType) => {
  const classes = {
    main: 'bg-slate-100 text-slate-800 dark:bg-slate-700 dark:text-slate-300',
    gold: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200',
    silver: 'bg-slate-100 text-slate-800 dark:bg-slate-900 dark:text-slate-200',
    bronze: 'bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-200',
    supporter: 'bg-slate-100 text-slate-800 dark:bg-slate-900 dark:text-slate-200'
  }
  return classes[sponsorType] || 'bg-slate-100 text-slate-800 dark:bg-slate-700 dark:text-slate-300'
}
