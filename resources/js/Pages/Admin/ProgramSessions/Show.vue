...
<template>
  <AdminLayout :page-title="safeProgramSession.title"
    :page-subtitle="`${safeProgramSession.venue?.display_name || 'Salon'} - ${formatEventInfo(safeProgramSession)}`"
    ...
    <div class="w-full space-y-8">
        ...
        <div class="relative">
              ...
              <div class="p-8 text-white w-full">
                <div class="flex items-end justify-between">
                  <div class="flex items-center space-x-4">
                    ...
                    <div>
                      <h1 class="text-3xl font-bold mb-1">{{ safeProgramSession.title }}</h1>
                      <p class="text-gray-100 text-lg">{{ safeProgramSession.formatted_time_range || 'Zaman belirtilmemiş' }}</p>
                    </div>
                  </div>
                  ...
                  <div class="flex items-center space-x-8 text-white/90">
                    <div class="text-center">
                      <div class="text-2xl font-bold">{{ safeProgramSession.presentations?.length || 0 }}</div>
                      <div class="text-sm">Sunum</div>
                    </div>
                    <div class="text-center">
                      <div class="text-2xl font-bold">{{ safeProgramSession.moderators?.length || 0 }}</div>
                      <div class="text-sm">Moderatör</div>
                    </div>
                    <div class="text-center">
                      <div class="text-2xl font-bold">{{ safeProgramSession.formatted_duration || '0 dk' }}</div>
          ...
          <div class="px-8 py-6 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
            <div class="flex flex-wrap items-center gap-6">
              ...
              <div class="flex items-center space-x-2">
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Tür:</span>
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium"
                  :class="getSessionTypeClasses(safeProgramSession.session_type)">
                  {{ safeProgramSession.session_type_display || safeProgramSession.session_type }}
                </span>
              </div>
              ...
              <div class="flex items-center space-x-2">
                ...
                <div>
                  <div class="text-sm font-medium text-gray-900 dark:text-white">{{
                    safeProgramSession.venue?.display_name || 'Salon Yok' }}</div>
                  <div class="text-xs text-gray-500 dark:text-gray-400">Salon</div>
                </div>
              </div>
              ...
              <div class="flex items-center space-x-2">
                ...
                <div>
                  <div class="text-sm font-medium text-gray-900 dark:text-white">{{ formatEventInfo(safeProgramSession)
                  }}</div>
              ...
              <!-- Break Status -->
              <div v-if="safeProgramSession.is_break" class="flex items-center space-x-2">
                <PauseIcon class="h-5 w-5 text-gray-400" />
                <div>
                  <div class="text-sm font-medium text-gray-600 dark:text-gray-400">Ara Oturumu</div>
                  <div class="text-xs text-gray-500 dark:text-gray-400">Özel Oturum</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="px-8 py-4 flex flex-wrap items-center justify-between gap-4 bg-white dark:bg-gray-900">
            <div class="flex items-center space-x-3">
              <!-- Back to List -->
              <Link :href="route('admin.program-sessions.index')"
            ...
            <div class="flex items-center space-x-3">
              ...
              <Link v-if="safeProgramSession.can_create_presentations"
                :href="route('admin.presentations.create', { program_session_id: safeProgramSession.id })"
              ...
              <!-- Edit Button -->
              <Link :href="route('admin.program-sessions.edit', safeProgramSession.id)"
      ...
      <div class="grid grid-cols-1 xl:grid-cols-4 gap-8">
        ...
        <div class="xl:col-span-3 space-y-8">
          <!-- Description -->
          <div v-if="safeProgramSession.description"
            ...
            <div class="prose dark:prose-invert max-w-none">
              <p class="text-gray-700 dark:text-gray-300 leading-relaxed">{{ safeProgramSession.description }}</p>
            </div>
          </div>
          ...
          <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
              ...
              <Link v-if="safeProgramSession.can_create_presentations"
                :href="route('admin.presentations.create', { program_session_id: safeProgramSession.id })"
                class="inline-flex items-center px-3 py-1.5 bg-gray-700 text-white text-sm font-medium rounded-lg hover:bg-gray-800 focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors">
            ...
            <!-- Presentations List -->
            <div v-if="safeProgramSession.presentations?.length > 0"
              class="divide-y divide-gray-200 dark:divide-gray-700">
              <div v-for="presentation in safeProgramSession.presentations" :key="presentation.id"
                ...
                <div class="flex items-start justify-between">
                  <div class="flex-1">
                    ...
                    <div class="flex items-center space-x-4 text-sm text-gray-500 dark:text-gray-400 mb-3">
                      ...
                      <div class="flex items-center">
                        ...
                        {{ presentation.speakers?.length || 0 }} konuşmacı
                      </div>
                    </div>

                    <div v-if="presentation.speakers?.length > 0" class="flex flex-wrap gap-2">
                      <span v-for="speaker in presentation.speakers" :key="speaker.id"
                  ...
                  <div class="flex items-center space-x-2 ml-4">
                    <Link :href="route('admin.presentations.show', presentation.id)"
                    ...
                    <Link :href="route('admin.presentations.edit', presentation.id)"
                      class="p-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-400 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
            ...
            <div v-else class="text-center py-16">
              ...
              <p class="text-gray-500 dark:text-gray-400 mb-6">Bu oturum için sunum eklemeye başlayın.</p>
              <Link v-if="safeProgramSession.can_create_presentations"
                :href="route('admin.presentations.create', { program_session_id: safeProgramSession.id })"
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Oturum Detayları</h3>
            <div class="space-y-4">
              <div class="flex items-center justify-between">
                ...
                <span class="text-sm font-semibold text-gray-900 dark:text-white">{{
                  formatTime(safeProgramSession.start_time) }}</span>
              </div>
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-2">
                  <ClockIcon class="h-5 w-5 text-gray-500" />
                  <span class="text-sm text-gray-600 dark:text-gray-400">Bitiş</span>
                </div>
                <span class="text-sm font-semibold text-gray-900 dark:text-white">{{
                  formatTime(safeProgramSession.end_time) }}</span>
              </div>
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-2">
                ...
                <span class="text-sm font-semibold text-gray-900 dark:text-white">{{
                  safeProgramSession.presentations?.length || 0 }}</span>
              </div>
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-2">
                  <UsersIcon class="h-5 w-5 text-gray-500" />
                  <span class="text-sm text-gray-600 dark:text-gray-400">Moderatör Sayısı</span>
                </div>
                <span class="text-sm font-semibold text-gray-900 dark:text-white">{{
                  safeProgramSession.moderators?.length || 0 }}</span>
              </div>
            </div>
          </div>

          <!-- Moderators -->
          <div v-if="safeProgramSession.moderators?.length > 0"
            class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Moderatörler</h3>
            <div class="space-y-3">
              <div v-for="moderator in safeProgramSession.moderators" :key="moderator.id"
                ...
                <div class="w-8 h-8 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center">
                  <UsersIcon class="w-4 h-4 text-gray-600 dark:text-gray-400" />
                </div>
                <div class="flex-1">
                  <div class="text-sm font-medium text-gray-900 dark:text-white">{{ moderator.full_name }}</div>
                  <div v-if="moderator.title" class="text-xs text-gray-500 dark:text-gray-400">{{ moderator.title }}
                  </div>
                  <div v-if="moderator.affiliation" class="text-xs text-gray-500 dark:text-gray-400">{{
                    moderator.affiliation }}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- Categories -->
          <div v-if="safeProgramSession.categories?.length > 0"
            class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Kategoriler</h3>
            <div class="flex flex-wrap gap-2">
              <span v-for="category in safeProgramSession.categories" :key="category.id"
          ...
          <!-- Sponsor -->
          <div v-if="safeProgramSession.sponsor"
            class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Sponsor</h3>
            <div class="flex items-center space-x-3">
              <div v-if="safeProgramSession.sponsor.logo_url"
                class="w-12 h-12 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800">
                <img :src="safeProgramSession.sponsor.logo_url" :alt="safeProgramSession.sponsor.name"
                  class="w-full h-full object-cover" />
              </div>
              <div class="flex-1">
                <div class="text-sm font-medium text-gray-900 dark:text-white">{{ safeProgramSession.sponsor.name }}
                </div>
                <div v-if="safeProgramSession.sponsor.website" class="text-xs text-gray-600 dark:text-gray-400">
                  <a :href="safeProgramSession.sponsor.website" target="_blank" class="hover:underline">
                    Website
                  </a>
                </div>
              </div>
    ...
    <ConfirmDialog v-model="confirmDialog.show" :title="confirmDialog.title" :message="confirmDialog.message"
      :type="confirmDialog.type" @confirm="confirmDialog.callback" />
  </AdminLayout>
</template>
...
const breadcrumbs = computed(() => {
  ...
  if (safeProgramSession.value.venue?.event_day?.event) {
    ...
    crumbs.splice(1, 0,
      ...
      { label: 'Günler', href: route('admin.events.days.index', event.slug) },
      { label: eventDay.display_name, href: route('admin.events.days.show', [event.slug, eventDay.id]) }
    )
  }
...
const duplicateSession = () => {
  ...
  if (safeProgramSession.value.id) {
    router.post(route('admin.program-sessions.duplicate', safeProgramSession.value.id), {}, {
      onSuccess: () => {
...
const deleteSession = () => {
  ...
  confirmDialog.value = {
    ...
    callback: () => {
      if (safeProgramSession.value.id) {
        router.delete(route('admin.program-sessions.destroy', safeProgramSession.value.id), {
          onSuccess: () => {
