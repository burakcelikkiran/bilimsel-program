...
<template>
  ...
  >
    ...
    <div class="w-full space-y-8">
        ...
        <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-gray-800 to-gray-900">
          <div class="flex items-center justify-between">
            <div class="flex items-center">
              ...
              <div class="ml-4">
                <h3 class="text-lg font-semibold text-white">Oturum Düzenle</h3>
                <p class="text-sm text-gray-300">{{ sessionTitle }}</p>
              </div>
            </div>
            <!-- Status Info -->
            <div class="flex items-center space-x-4">
              <div class="text-right">
                <p class="text-sm font-medium text-white">{{ selectedVenue?.display_name || 'Salon Seçiniz' }}</p>
              ...
              >
                ...
                {{ form.is_break ? 'Ara' : form.session_type || 'Oturum' }}
              </span>
            </div>
          </div>
        </div>
      ...
      <!-- Form Card -->
      <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <form @submit.prevent="updateSession" class="divide-y divide-gray-200 dark:divide-gray-700">
          
          <!-- Event Context Selection -->
          <div class="p-6 space-y-6 bg-gray-50 dark:bg-gray-800/50">
            <div>
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Etkinlik Seçimi</h3>
              ...
              <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  >
                    <option value="">{{ isLoadingEventDays ? 'Yükleniyor...' : 'Gün seçin' }}</option>
                ...
                <div>
                  ...
                  <select
                    ...
                    required
                    class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-gray-500 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md focus:shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                    :class="errors.venue_id ? 'border-red-300 focus:ring-red-500' : ''"
                  >
                    <option value="">{{ isLoadingVenues ? 'Yükleniyor...' : 'Salon seçin' }}</option>
                    <option v-for="venue in availableVenues" :key="venue.id" :value="venue.id">
                      {{ venue.display_name }}
                    </option>
                  </select>
                  <p v-if="errors.venue_id" class="mt-2 text-sm text-red-600">{{ errors.venue_id }}</p>
                  <!-- Debug Info -->
                  <div v-if="debugMode" class="mt-2 p-2 bg-blue-50 rounded text-xs">
                    <strong>DEBUG:</strong><br>
                    selectedEventDayId: {{ selectedEventDayId }}<br>
              ...
              <div class="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <div class="lg:col-span-3 xl:col-span-2">
                  ...
                  <input
                    ...
                    :class="errors.title ? 'border-red-300 focus:ring-red-500' : ''"
                ...
                <div class="lg:col-span-1 xl:col-span-1">
                  ...
                  <select
                    ...
                    required
                    class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-gray-500 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md focus:shadow-md"
                    :class="errors.session_type ? 'border-red-300 focus:ring-red-500' : ''"
                  >
                    <option value="">Oturum türü seçin</option>
                    <option v-for="type in sessionTypes" :key="type.value" :value="type.value">
                      {{ type.label }}
                    </option>
                  </select>
                  <p v-if="errors.session_type" class="mt-2 text-sm text-red-600">{{ errors.session_type }}</p>
                </div>
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  <input
                    ...
                    :class="errors.start_time ? 'border-red-300 focus:ring-red-500' : ''"
                  />
                </div>
                <p v-if="errors.start_time" class="mt-2 text-sm text-red-600">{{ errors.start_time }}</p>
              </div>
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  <input
                    ...
                    required
                    class="block w-full pl-10 pr-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-gray-500 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md focus:shadow-md"
                    :class="errors.end_time ? 'border-red-300 focus:ring-red-500' : ''"
                    :min="form.start_time"
                  />
                </div>
                <p v-if="errors.end_time" class="mt-2 text-sm text-red-600">{{ errors.end_time }}</p>
              </div>
            ...
            <div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                  ...
                  >
                    <option value="">Moderatör unvanı seçin</option>
                ...
                <div class="md:col-span-2">
                  ...
                  <p v-if="errors.moderator_ids" class="text-sm text-red-600 dark:text-red-400 mt-1">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div v-else class="text-center py-4 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-600 rounded-lg">
                    ...
                    <p class="text-sm">{{ selectedEventId ? 'Bu etkinlik için kategori yok' : 'Önce etkinlik seçin' }}</p>
                  </div>
                  
                  <p v-if="errors.category_ids" class="text-sm text-red-600 dark:text-red-400 mt-1">
                    {{ errors.category_ids }}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-gray-500 focus:border-transparent transition-all duration-200 shadow-sm hover:shadow-md focus:shadow-md"
                    :class="errors.sponsor_id ? 'border-red-300 focus:ring-red-500' : ''"
                  >
                    <option value="">Sponsor seçin (opsiyonel)</option>
                    <option v-for="sponsor in sponsors" :key="sponsor.id" :value="sponsor.id">
                      {{ sponsor.name }}
                    </option>
                  </select>
                  <p v-if="errors.sponsor_id" class="mt-2 text-sm text-red-600">{{ errors.sponsor_id }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center justify-between p-6 border-t border-gray-200 dark:border-gray-700">
            <Link
              :href="sessionId ? route('admin.program-sessions.show', sessionId) : route('admin.program-sessions.index')"
      ...
      <div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
        <div class="flex items-start">
          ...
          <div class="ml-3">
            <h4 class="text-sm font-medium text-gray-900 dark:text-gray-100">Program Oturum Düzenleme İpuçları</h4>
            <div class="mt-2 text-sm text-gray-700 dark:text-gray-200">
              <ul class="list-disc list-inside space-y-1">
                <li>Etkinlik, gün ve salon değişikliği yapabilirsiniz</li>
                <li>Zaman değişikliği yaparken çakışma kontrolü yapın</li>
                <li>Moderatör seçimini oturum türüne göre yapın</li>
                <li>Değişiklikleri kaydetmeyi unutmayın</li>
                <li>Açıklama kısmında güncel bilgileri tutun</li>
              </ul>
            </div>
          </div>
        </div>
...
// Reactive state for cascade selection
const selectedEventId = ref(null)
const selectedEventDayId = ref(null)
const availableEventDays = ref([])
...
const safeProgramSession = computed(() => {
  ...
  return {
    ...
    moderator_ids: Array.isArray(session.moderator_ids) ? session.moderator_ids : [],
    category_ids: Array.isArray(session.category_ids) ? session.category_ids : [],
    venue: {
      id: session.venue?.id || null,
      name: session.venue?.name || session.venue?.display_name || 'Salon',
      event_day: {
        id: session.venue?.event_day?.id || null,
        title: session.venue?.event_day?.title || session.venue?.event_day?.display_name || 'Gün',
        event: {
...
const selectedEventDay = computed(() => {
  return availableEventDays.value.find(day => day.id == selectedEventDayId.value)
})
...
const breadcrumbs = computed(() => {
  const crumbs = [{ label: 'Etkinlikler', href: route('admin.events.index') }]
  ...
  crumbs.push(
    ...
    { label: sessionTitle.value, href: sessionId.value ? route('admin.program-sessions.show', sessionId.value) : null },
...
const hasChanges = computed(() => {
  return Object.keys(originalForm).some(key => {
    if (Array.isArray(form[key])) {
      return JSON.stringify(form[key]) !== JSON.stringify(originalForm[key])
    }
...
const onEventChange = async () => {
  console.log('🔥 onEventChange called, selectedEventId:', selectedEventId.value)
  ...
  try {
    ...
    if (response.data.success) {
      ...
      console.log('✅ Event days loaded:', availableEventDays.value.length, 'days')
      
      // Load categories for selected event
      await loadCategoriesForEvent()
    } else {
      console.error('❌ Event days loading failed:', response.data.message)
    }
  } catch (error) {
    ...
    if (error.response) {
      ...
      alert(`Hata: ${error.response.status} - ${error.response.data.message || 'Sunucu hatası'}`)
    } else {
...
const onEventDayChange = async () => {
  console.log('🔥 onEventDayChange called, selectedEventDayId:', selectedEventDayId.value)
  ...
  try {
    ...
    if (response.data.success) {
      ...
      console.log('✅ Venues loaded:', availableVenues.value.length, 'venues')
    } else {
    ...
    if (error.response) {
      alert(`Salon yükleme hatası: ${error.response.status} - ${error.response.data.message || 'Sunucu hatası'}`)
    }
  } finally {
...
const loadCategoriesForEvent = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      console.log('✅ Categories loaded:', availableCategories.value.length, 'categories')
    } else {
      console.error('❌ Categories loading failed:', response.data.message)
    }
  } catch (error) {
...
const updateSession = async () => {
  ...
  try {
    await router.put(route('admin.program-sessions.update', sessionId.value), form, {
      onSuccess: () => {
...
const resetForm = () => {
  ...
  selectedEventId.value = safeProgramSession.value.venue?.event_day?.event?.id || null
  selectedEventDayId.value = safeProgramSession.value.venue?.event_day?.id || null
...
const initializeCascadeSelections = () => {
  ...
  // Set initial values from current session
  selectedEventId.value = safeProgramSession.value.venue?.event_day?.event?.id || null
  selectedEventDayId.value = safeProgramSession.value.venue?.event_day?.id || null
...
// Lifecycle
onMounted(() => {
  console.log('✅ ProgramSession edit form with full cascade selection loaded')
  console.log('📋 Props:', {
