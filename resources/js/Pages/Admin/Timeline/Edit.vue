  ...
  >
    ...
    <div class="mb-6">
      <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-4">
              <div class="h-12 w-12 bg-orange-600 rounded-xl flex items-center justify-center">
                <CursorArrowRaysIcon class="h-8 w-8 text-white" />
              </div>
              <div>
                ...
                <p class="text-sm text-orange-600 dark:text-orange-400">
                  ...
                  Düzenleme Modu - Oturumları sürükleyerek yeniden düzenleyebilirsiniz
                </p>
              </div>
            </div>
        ...
        <!-- Change Summary -->
        <div v-if="hasUnsavedChanges" class="px-6 py-4 bg-orange-50 dark:bg-orange-900/20">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-2">
              <ClockIcon class="h-5 w-5 text-orange-600" />
              <span class="text-sm font-medium text-orange-800 dark:text-orange-200">
                {{ pendingChanges.length }} kaydedilmemiş değişiklik
              </span>
            </div>
            <button
          ...
          <div class="mt-3 space-y-1">
            ...
            <div v-if="pendingChanges.length > 3" class="text-xs text-orange-600 dark:text-orange-400">
              ... ve {{ pendingChanges.length - 3 }} değişiklik daha
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="space-y-8">
      ...
      <!-- Simple Timeline Placeholder (DraggableTimelineContainer component henüz yok) -->
      <div v-else class="bg-white shadow rounded-lg">
        <div class="p-6">
          <div v-if="timelineData.length === 0" class="text-center py-12">
            ...
            <p class="text-gray-500 dark:text-gray-400">Bu etkinlik için henüz program oluşturulmamış.</p>
            <Link
          ...
          <div v-else>
            <!-- Basit Timeline Gösterimi -->
            <div class="border-2 border-dashed border-gray-300 rounded-lg p-12 text-center">
              <h3 class="text-lg font-medium text-gray-900 mb-2">
                Drag & Drop Timeline
              </h3>
              <p class="text-gray-500 mb-4">
                Timeline bileşeni yakında eklenecek
              </p>
              <div class="text-sm text-gray-400">
                • {{ timelineData.length }} gün
              </div>
            </div>
          </div>
        </div>
    ...
    <div v-if="conflicts.length > 0" class="mt-8">
      <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-6">
        <div class="flex items-start space-x-3">
          <ExclamationTriangleIcon class="h-6 w-6 text-red-500 mt-0.5 flex-shrink-0" />
          <div class="flex-1">
            ...
            <div class="space-y-2">
              ...
              >
                ...
                <div class="text-sm text-red-600 dark:text-red-300 mt-1">
                  "{{ conflict.session1_title }}" ile "{{ conflict.session2_title }}" arasında 
                  {{ conflict.overlap_minutes }} dakika çakışma
                </div>
              </div>
            </div>
          </div>
    ...
    <div class="mt-8">
      <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-6">
        <div class="flex items-start space-x-3">
          <InformationCircleIcon class="h-6 w-6 text-blue-500 mt-0.5 flex-shrink-0" />
          <div>
            <h3 class="text-lg font-semibold text-blue-800 dark:text-blue-200 mb-2">
              Sürükle-Bırak Kullanım Rehberi
            </h3>
            <div class="text-sm text-blue-700 dark:text-blue-300 space-y-1">
              <div>• Oturumları sürükleyerek farklı salonlara veya zamanlara taşıyabilirsiniz</div>
              <div>• Yeşil alan: Geçerli bırakma alanı</div>
              <div>• Kırmızı alan: Çakışma nedeniyle geçersiz alan</div>
              <div>• Mavi alan: Oturumun mevcut konumu</div>
              <div>• Değişiklikler otomatik olarak kaydedilmez, "Kaydet" butonuna basmanız gerekir</div>
            </div>
          </div>
        </div>
      </div>
...
// DraggableTimelineContainer component'i henüz yok, o yüzden import etmiyoruz
import {
  CursorArrowRaysIcon,
...
const discardChanges = () => {
  if (confirm('Tüm kaydedilmemiş değişiklikler iptal edilecek. Emin misiniz?')) {
    router.reload()
  }
}
...
const beforeUnloadHandler = (event) => {
  if (hasUnsavedChanges.value) {
    ...
    event.returnValue = 'Kaydedilmemiş değişiklikler var. Sayfadan çıkmak istediğinizden emin misiniz?'
