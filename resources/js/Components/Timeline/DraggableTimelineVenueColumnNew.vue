...
<template>
    <div class="draggable-timeline-venue-column">
        ...
        <div class="venue-sessions">
            ...
            >
                <li
                    v-for="(session, index) in dragList"
                    :key="session.id"
                    class="session-item"
                >
                    <DraggableTimelineSessionCard
                        ...
                        :event="event"
                        :sort-order="index"
                        :is-last-session="index === dragList.length - 1"
        ...
        <div v-if="dragList.length > 0" class="venue-summary mt-4">
            <div
...
// FormKit drag and drop setup
const sessionsList = ref(null);
const [dragList, isDragActive] = useDragAndDrop(
...
// Watch for drag list changes
watch(dragList, (newList) => {
    // Update sort orders
    newList.forEach((session, index) => {
    ...
    emit("session-updated", {
        session_order: newList.map(s => s.id),
...
const timeConflicts = computed(() => {
    ...
    for (let i = 0; i < dragList.value.length; i++) {
        for (let j = i + 1; j < dragList.value.length; j++) {
            const session1 = dragList.value[i];
            const session2 = dragList.value[j];
...
const venueTotalDuration = computed(() => {
    let totalMinutes = 0;
    dragList.value.forEach(session => {
...
const venueTimeRange = computed(() => {
    ...
    const times = dragList.value
        .filter(s => s.start_time)
        .map(s => s.start_time)
        .sort();
    
    const endTimes = dragList.value
        .filter(s => s.end_time)
        .map(s => s.end_time)
        .sort();
...
const calculateNewTimes = () => {
    ...
    dragList.value.forEach((session, index) => {
        if (index === 0) {
            currentTime = session.start_time || "09:00";
        } else {
            const prevSession = dragList.value[index - 1];
...
const handleMoveDown = (session, currentIndex) => {
    if (currentIndex < dragList.value.length - 1) {
        const newList = [...dragList.value];
