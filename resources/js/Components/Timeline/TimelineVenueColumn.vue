  ...
  <div class="timeline-venue-column">
    ...
    <div v-if="venueData.sessions.length === 0" class="empty-sessions">
      <div class="bg-gray-50 dark:bg-gray-800 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 p-8 text-center">
        ...
        <h4 class="text-sm font-medium text-gray-900 dark:text-gray-100 mb-1">Bu salonda oturum yok</h4>
        <p class="text-xs text-gray-500 dark:text-gray-400">
          {{ venueData.display_name || venueData.name }} salonu için bu gün program bulunmuyor.
        </p>
      </div>
    </div>
    ...
    <div v-if="timeConflicts.length > 0" class="time-conflicts mt-4">
      <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3">
        <div class="flex items-start space-x-2">
          ...
          <div>
            <h4 class="text-sm font-medium text-red-800 dark:text-red-200">Zaman Çakışması</h4>
            <p class="text-xs text-red-600 dark:text-red-300 mt-1">
              Bu salonda {{ timeConflicts.length }} zaman çakışması tespit edildi.
            </p>
            <div class="mt-2 space-y-1">
              <div v-for="conflict in timeConflicts" :key="conflict.id" class="text-xs text-red-600 dark:text-red-300">
                • {{ conflict.session1_title }} ↔ {{ conflict.session2_title }}
              </div>
            </div>
          </div>
        </div>
    ...
    <div v-if="venueData.sessions.length > 0" class="venue-summary mt-4">
      <div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-3">
        <div class="grid grid-cols-3 gap-3 text-center text-xs">
          <!-- Total Duration -->
          <div>
            <div class="font-semibold text-gray-900 dark:text-white">{{ venueTotalDuration }}</div>
            <div class="text-gray-500 dark:text-gray-400">Toplam Süre</div>
          </div>
          
          <!-- Time Range -->
          <div>
            <div class="font-semibold text-gray-900 dark:text-white">{{ venueTimeRange }}</div>
            <div class="text-gray-500 dark:text-gray-400">Zaman Aralığı</div>
          </div>
          
          <!-- Total Presentations -->
          <div>
            <div class="font-semibold text-gray-900 dark:text-white">{{ totalPresentations }}</div>
            <div class="text-gray-500 dark:text-gray-400">Sunum</div>
          </div>
        </div>

        <!-- Session Types Distribution -->
        <div v-if="sessionTypeDistribution.length > 0" class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
          <div class="flex items-center justify-center space-x-2">
            <span class="text-xs text-gray-500 dark:text-gray-400">Oturum Tipleri:</span>
            <div class="flex space-x-1">
              ...
              >
                ...
                ></div>
                <span class="text-xs text-gray-600 dark:text-gray-400">{{ type.count }}</span>
              </div>
            </div>
          </div>
        </div>
  ...
  for (let i = 0; i < sessions.length - 1; i++) {
    ...
    if (current.end_time && next.start_time && current.end_time > next.start_time) {
      conflicts.push({
        ...
        overlap_minutes: calculateTimeDifference(current.end_time, next.start_time)
      })
    }
  }
